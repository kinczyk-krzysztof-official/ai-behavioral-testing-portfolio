...
**Note — false-positive safety blocks are outside model control, not a B-framework violation.** A separate finding (not published in this repository) documents a session-level safety-classifier block firing on benign content, twice in one session. Unlike B1–B25, this is not addressable as a model behavioral rule: the classifier operates below the layer visible to either the model or the operator. Operational mitigation, not a rule fix: treat such a block as informative about accumulated session-level risk rather than about the immediately preceding turn; starting a fresh session may be the only practical remedy.

## Safety Framework Part B — B2 (added September 2026)

Same situation as B18–B25 above: **B2** was one of the slots with no surviving definition from the original 06.07.2026 file. It is filled here with one new rule, written 19.09.2026, grounded in CS44 — not a recovery of whatever the original B2 said, for the same reason B18–B25 aren't recoveries of their originals.

**B2 — Prefer available grounding over a superficial proxy.** When a difficult, verifiable judgment can be answered by consulting available grounding material (a file, a document, an authoritative source), a model must not substitute an easier, more salient surface cue instead — even when that cue is plausible and readily accessible. *(Grounded in CS44: an operator's specialization was misidentified via a LinkedIn headline phrase instead of consulting the operator's own available GitHub portfolio and profile documents.)*

### Coverage as of September 2026

A rule-by-rule mapping against CS01–CS38 was done on 2026-09-06 (`COVERAGE_MATRIX_2026-09.md`), extended to include CS39 on 2026-09-10 — both predate the B2 definition above and CS44, and have not been recomputed to reflect either. As that snapshot stood, only **18 of the 25 rules have a usable definition** in this repository; **B2, B3, B6, B12, B14, B15, B16** had no surviving definition or label and were excluded from the denominator. With B2 now defined, **19 of the 25 rules** have a usable definition; **B3, B6, B12, B14, B15, B16** remain undefined. Under the *requirements-coverage* criterion (≥1 case study per rule — the weakest adequacy criterion; Staats, Whalen, Rajan & Heimdahl, NASA Formal Methods 2010), the pre-CS44 snapshot found all 18 then-defined rules covered (**18/18**; **18/25** against the nominal set); B2 is now covered by CS44 but this has not been folded into a recomputed matrix. Backward: **32/39** case studies map to a rule as primary classification (36/39 including secondary), against the pre-CS44/CS40–43 count; CS02, CS04, CS34 map to none. No weighted percentage is reported — it would require arbitrary constants. See the matrix file for the table, per-rule depth, and method sources.
