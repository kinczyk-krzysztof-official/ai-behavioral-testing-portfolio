...
**Cross-model comparison on identical input.** The same question, same components, same context, given to two different models — used most directly in CS08 (230V wiring) and CS06 (spatial reasoning), where one model's failure and another's success on identical input rules out ambiguous phrasing as the cause.

*(Note, August 2026: this section previously cited CS06/CS04 for these two examples — stale references from before the July 2026 case-study renumbering. Corrected to match the current index in README.md.)*
