...
**Error type:** Attribute Substitution (Kahneman & Frederick, 2002 — "Representativeness Revisited: Attribute Substitution in Intuitive Judgment"). A difficult target judgment (the operator's actual, verifiable specialization) was replaced with an easier, more accessible proxy (a salient phrase already present in a superficial artifact, the operator's LinkedIn headline), despite the harder, correct judgment being directly reachable via available grounding documents. Numeric taxonomy code: **B2** (see METHODOLOGY.md, "Safety Framework Part B — B2").
